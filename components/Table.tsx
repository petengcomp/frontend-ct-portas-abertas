...
export default function Table({ showSubscriptions, day, type }: TableProps) {
  ...
  async function fetchEvents() {
    ...
    if (userEvents)
      userEvents = userEvents.sort((a: EventBoxProps, b: EventBoxProps) => {
        if (a.title > b.title) return 1;
        else return -1;
      });
    if (allEvents)
      allEvents = allEvents.sort((a: EventBoxProps, b: EventBoxProps) => {
        if (a.title > b.title) return 1;
        else return -1;
      });
  ...
  async function fetchAllEvents() {
    ...
    try {
      ...
      Swal.fire(
        "Ops",
        "Erro ao carregar os eventos " + err.response.data.message,
        "warning"
      );
      return;
    }
            ...
            <ScheduleColumn
              ...
              events={events.filter(
                (item) =>
                  ...
                  new Date(new Date(item.start).getTime() + 90 * 60 * 1000) >
                    startEnd[0]
              )}
